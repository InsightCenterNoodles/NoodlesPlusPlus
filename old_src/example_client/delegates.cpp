...
QVariant ExMethod::get_column(int c) const {
    ...
    case 4: return m_argument_names;
    }
    return {};
}
...
Qt3DRender::QBuffer* ExBuffer::entity() {
    if (!m_3d_entity) {
        m_3d_entity = std::make_unique<Qt3DRender::QBuffer>(m_scene_root);

        // doesnt matter if this ready or not;
        m_3d_entity->setData(m_array);
    }

    return m_3d_entity.get();
}
...
QtGeomInfo ExMesh::make_new_info(std::span<glm::mat4> instances) {
    ...
    // set up geometry
    ret.geom = std::make_unique<Qt3DRender::QGeometry>(m_scene_root);
    ret.renderer =
        std::make_unique<Qt3DRender::QGeometryRenderer>(m_scene_root);
    ...
    auto* ca = attrib_from_ref<glm::u8vec4>(
        QAttribute::defaultColorAttributeName(), ret.geom.get(), m_data.colors);

    QAttribute* idx_p = nullptr;

    if (m_data.triangles) {
        idx_p =
            attrib_from_idx_ref<glm::u16vec3>(ret.geom.get(), m_data.triangles);

        ret.renderer->setPrimitiveType(QGeometryRenderer::Triangles);

    } else if (m_data.lines) {
        idx_p = attrib_from_idx_ref<glm::u16vec2>(ret.geom.get(), m_data.lines);
    ...
    ret.instance_buffer = std::make_unique<Qt3DRender::QBuffer>(m_scene_root);

    ret.instance_buffer->setData(ret.instance_data);

    ret.instance_attribute.reset(
        instance_attrib(ret.geom.get(), ret.instance_buffer.get()));

    ret.geom->addAttribute(ret.instance_attribute.get());

    // now we need another attribute to get proper cull bounds from instances
    auto* fake_attrib =
        instance_fake_pos_attrib(ret.geom.get(), ret.instance_buffer.get());

    ret.geom->addAttribute(fake_attrib);
    ret.geom->setBoundingVolumePositionAttribute(fake_attrib);

    ret.renderer->setGeometry(ret.geom.get());
...
void ExObject::remake_mesh_attachment() {
    if (m_att_mesh_details) {
        m_3d_entity->removeComponent(m_att_mesh_details->renderer.get());
    }
    ...
    if (m_att_mesh_details) {
        m_3d_entity->addComponent(m_att_mesh_details->renderer.get());
    }
}
...
void ExObject::update_from(nooc::ObjectUpdateData const& md) {
    ...
    if (md.name) {
        ...
        m_tree_item->setText(0, get_name());
    }
    ...
    if (md.parent) {
        ...
        // unhook current
        if (m_parent) {
            auto* p_tree_item = m_parent->m_tree_item.get();

            Q_ASSERT(p_tree_item);

            qDebug() << "Current parent text"
                     << p_tree_item->data(0, Qt::DisplayRole);

            auto index = p_tree_item->indexOfChild(m_tree_item.get());

            auto* c = p_tree_item->takeChild(index);

            Q_ASSERT(c);
            Q_ASSERT(c == m_tree_item.get());
        }
        ...
        if (m_parent) {
            ...
            qDebug() << "Assign parent to"
                     << p_tree_item->data(0, Qt::DisplayRole);

            p_tree_item->addChild(m_tree_item.get());

        } else {

            qDebug() << "Assign parent to"
                     << m_tree_root->data(0, Qt::DisplayRole);
            m_tree_root->addChild(m_tree_item.get());
        }
...
ExObject::ExObject(noo::ObjectID                 id,
    ...
    : nooc::ObjectDelegate(id, md),
      ...
      m_tree_item(new QTreeWidgetItem(tree_root)),
    ...
    // TODO: Improve tree widget stuff. storing an id as a string??

    m_tree_item->setText(0, get_name());
    m_tree_item->setText(1, QString::number(id.id_slot));
    m_tree_item->setText(2, QString::number(id.id_gen));
}

ExObject::~ExObject() {

    if (m_3d_entity) { delete m_3d_entity.data(); }
