...
void PendingMethodReply::complete(noo::AnyVar v, QString string) {
    qDebug() << Q_FUNC_INFO;
...
void GetStringReply::interpret() {
    ...
    VMATCH(
        m_var,
        VCASE(std::string & str) { emit recv(QString::fromStdString(str)); },
        VCASE(auto&) {
            auto s = m_var.dump_string();
...
void GetStringListReply::interpret() {
    VMATCH(
        ...
        VCASE(noo::AnyVarList & l) {
            ...
            for (auto elem : l) {
                to_ret << QString::fromStdString(elem.steal_string());
            }
...
void GetSelectionReply::interpret() {
    auto sel = noo::Selection(std::move(m_var));
...
void TableDelegate::request_deletion(noo::SelectionRef selection) const {
