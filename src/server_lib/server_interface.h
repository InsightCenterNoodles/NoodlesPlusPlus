...
///
/// \brief The TableQuery struct represents a query on a given table.
///
struct TableQuery {
    virtual ~TableQuery();
...
class TableSource : public QObject {
    ...
    // non const spans permit modification
protected:
...
public:
    ...
    /// Provide the number of rows of your table.
    ...
    bool trigger_set_selection(std::string_view selection_id, SelectionRef);
